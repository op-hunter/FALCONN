 ...
 */
void GenQueries(vector<Point> *dataset, vector<Point> *queries, faiss::ConcurrentBitsetPtr bitset = nullptr) {
  mt19937_64 gen(SEED);
  queries->clear();
...
pair<double, QueryStatistics> evaluate_query_time(
    ...
    const vector<int> &answers, int num_probes, faiss::ConcurrentBitsetPtr bitset = nullptr) {
  unique_ptr<LSHNearestNeighborQuery<Point>> query_object =
      table->construct_query_object(num_probes);
...
//    auto ans = query_object->find_nearest_neighbor(query);
    query_object->find_k_nearest_neighbors(query, topk, &qa, bitset);
...
//    read_dataset(FILE_NAME, &dataset);
    ...
    //init bitset
    faiss::ConcurrentBitsetPtr bitset = std::make_shared<faiss::ConcurrentBitset>(n_total + 1);
    // selecting NUM_QUERIES data points as queries
    cout << "selecting " << NUM_QUERIES << " queries" << endl;
//    gen_queries(&dataset, &queries);
    GenQueries(&dataset, &queries, bitset);
    cout << "done" << endl;
...
//    cout << num_probes << " probes" << endl;
    ...
    // statistics
    auto tmp = evaluate_query_time(&*table, queries, answers, num_probes, bitset);
